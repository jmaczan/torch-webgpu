#!/usr/bin/env python
"""
Benchmark script for Qwen2.5-0.5B-Instruct using ONNX Runtime with WebGPU.

This benchmarks the ONNX model with WebGPUExecutionProvider via optimum.
"""
...
try:
    ...
    HAS_ORT = False
    print("Error: onnxruntime-webgpu not installed. Run: pip install onnxruntime-webgpu")
...
def benchmark_onnx_webgpu(
    ...
    """Benchmark ONNX model with WebGPU provider using optimum."""
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Qwen ONNX model with WebGPU")
    ...
    if not HAS_OPTIMUM:
        print("Error: optimum package required. Run: pip install optimum[onnxruntime]")
        sys.exit(1)

    if verbose:
        print("=" * 60)
        print("Qwen2.5-0.5B-Instruct ONNX Runtime WebGPU Benchmark")
        print("=" * 60)
        print()
    ...
    if "WebGpuExecutionProvider" not in available:
        print("Error: WebGpuExecutionProvider not available")
        print("Install: pip install onnxruntime-webgpu")
        sys.exit(1)
    ...
    if not model_dir.exists() or not list(model_dir.glob("*.onnx")):
        if verbose:
            ...
            print("Please export the model first using: python benchmarks/export_qwen_onnx.py")
        sys.exit(1)

    # Load model with WebGPU provider
    if verbose:
        print(f"Loading ONNX model from {model_dir} with WebGPU provider...")
    ...
    results["backend"] = "onnxruntime-webgpu"
    results["provider"] = "WebGpuExecutionProvider"
    ...
    print("RESULTS")
    print("=" * 60)
    print(f"Provider:            WebGpuExecutionProvider")
